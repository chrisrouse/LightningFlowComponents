...
describe("hide binding", () => {
    it("takes a Boolean resource and refuses a literal", () => {
        const picker = control(build(), "hideWhen");
    ...
    it("publishes the reference the admin picked", async () => {
        ...
        control(element, "hideWhen").dispatchEvent(
            new CustomEvent("resourcechange", {
                detail: { name: "hideWhen", newValue: "{!dismissed}", newValueDataType: "reference" }
            })
        );
        await Promise.resolve();

        expect(details).toEqual([{ name: "hideWhen", newValue: "{!dismissed}", newValueDataType: "reference" }]);
    });

    it("reads a saved reference back in {!...} form", () => {
        const element = build({
            inputVariables: [{ name: "hideWhen", value: "dismissed", valueDataType: "reference" }]
        });

        expect(control(element, "hideWhen").value).toBe("{!dismissed}");
    });

    it("still offers the expiry checkbox alongside it", () => {
        const element = build();
        expect(control(element, "hideOnExpiry").type).toBe("checkbox");
    });
});
