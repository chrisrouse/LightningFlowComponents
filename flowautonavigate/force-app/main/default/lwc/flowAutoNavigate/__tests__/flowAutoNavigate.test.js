...
describe("hide when this is true", () => {
    function content(element) {
        return element.shadowRoot.querySelector(".auto-navigate__content");
    }

    function isHidden(element) {
        return content(element).classList.contains("auto-navigate__content_hidden");
    }

    it("hides immediately when the bound value is already true", async () => {
        const element = build({ seconds: 10, showTimer: true, hideWhen: true });
        await Promise.resolve();

        expect(isHidden(element)).toBe(true);
    });

    it("hides and reappears as the bound value changes", async () => {
        const element = build({ seconds: 30, showTimer: true });
        await Promise.resolve();
        expect(isHidden(element)).toBe(false);

        element.hideWhen = true;
        await Promise.resolve();
        expect(isHidden(element)).toBe(true);

        element.hideWhen = false;
        await Promise.resolve();
        expect(isHidden(element)).toBe(false);
    });

    /**
     * Hiding is visual only. Someone who wants the clock held as well should
     * bind `paused`; the two are deliberately separate.
     */
    it("keeps running and still advances while hidden", () => {
        const element = build({ seconds: 10, showTimer: true, hideWhen: true });
        const events = captureEvents(element);
    ...
    it("keeps its footprint so nothing below it moves", async () => {
        const element = build({ seconds: 10, showTimer: true, hideWhen: true });
        await Promise.resolve();

        // Still rendered, just invisible -- display:none would collapse it.
        expect(content(element)).not.toBeNull();
    });

    it("stays hidden on expiry when the bound value is still true", async () => {
        const element = build({ seconds: 10, showTimer: true, hideWhen: true, timeoutAction: "Stay" });
    ...
    it("combines with Hide When Time Runs Out rather than overriding it", async () => {
        const element = build({
            seconds: 10,
            showTimer: true,
            hideOnExpiry: true,
