...
describe("Flex depends on Width", () => {
    // Flex picks between initialWidth (resizable) and fixedWidth (locked). With no
    // width there is nothing to pick, so the checkbox does nothing — and it ships
    // checked, which made it look as though it did. Six browser configurations were
    // tested before that became obvious, so the dependency is now visible in the UI.
    it("disables Flex while the column has no width", async () => {
        const element = build({ columnFields: '["Name"]' });
        await Promise.resolve();

        const flex = cell(element, "Name", "flex");
        expect(flex.disabled).toBe(true);
        expect(flex.title).toContain("Set a width first");
    });

    it("enables Flex once a width is set", async () => {
        const element = build({
            columnFields: '["Name"]',
            columnConfig: JSON.stringify({ Name: { width: 300 } })
        });
        await Promise.resolve();
    ...
    it.each([0, null, "", "abc"])("treats %p as no width", async (width) => {
        const element = build({
            columnFields: '["Name"]',
            columnConfig: JSON.stringify({ Name: { width } })
        });
        await Promise.resolve();

        expect(cell(element, "Name", "flex").disabled).toBe(true);
    });
});
